...
EditableGrid.prototype.initializeGrid = function() 
{
	with (this) {
		...
		setHeaderRenderer("criterio", new InfoHeaderRenderer("Criterio de actualización para esta vista"));
		...
		getColumn("criterio").cellEditor.minWidth = 300;
		...
		getColumn("valido").cellEditor.minWidth = 75;

		// use a flag image to render the selected country
		setCellRenderer("archivos", new CellRenderer({render: function(cell, value) { 
			var estado = editableGrid.getValueAt(cell.rowIndex,5);

			if (estado.indexOf("Error")>0) {
			   var rutaArchivon3 = "publicacion/Error/" + value.toLowerCase() + "/datos.n3";
			   var rutaArchivon3 = btoa(encodeURIComponent(rutaArchivon3));
			   cell.innerHTML = value ? "<a target='_blank' href='publicacion/Error/" + value.toLowerCase() + "/datos.csv' alt='datos.csv'><img border='0' alt='csv' title=\"Ver Archivo de carga CSV\" src='" + image("excel.png")+ "'></a>&nbsp" +
									    "<a target='_blank' href='/pool-administracion-cargas/cargavistas/verarchivo/" + rutaArchivon3 + "' alt='datos.n3'><img border='0' alt='n3'  title=\"Ver Archivo de carga N3\" src='" + image("file.png") + "'/></a>" : ""; 
			} else if (estado.indexOf("Sin Procesar")>0) {
				var rutaArchivon3 = "publicacion/NoProcesados/" + value.toLowerCase() + "/datos.n3";
				var rutaArchivon3 = btoa(encodeURIComponent(rutaArchivon3));
			   cell.innerHTML = value ? "<a target='_blank' href='publicacion/NoProcesados/" + value.toLowerCase() + "/datos.csv' alt='datos.csv'><img border='0' alt='csv' title=\"Ver Archivo de carga CSV\"  src='" + image("excel.png")+ "'></a>&nbsp" +
			                            "<a target='_blank' href='/pool-administracion-cargas/cargavistas/verarchivo/" + rutaArchivon3 + "' alt='datos.n3'><img border='0' alt='n3'  title=\"Ver Archivo de carga N3\" src='" + image("file.png") + "'/></a>" : ""; 
			} else {
				var rutaArchivon3 = "publicacion/Procesados/" + value.toLowerCase() + "/datos.n3";
				var rutaArchivon3 = btoa(encodeURIComponent(rutaArchivon3));
				cell.innerHTML = value ? "<a target='_blank' href='publicacion/Procesados/" + value.toLowerCase() + "/datos.csv' alt='datos.csv'><img border='0' alt='csv' title=\"Ver Archivo de carga CSV\"  src='" + image("excel.png")+ "'></a>&nbsp" +
				                         "<a target='_blank' href='/pool-administracion-cargas/cargavistas/verarchivo/" + rutaArchivon3 + "' alt='datos.n3'><img border='0' alt='n3' title=\"Ver Archivo de carga N3\"  src='" + image("file.png") + "'/></a>" : "";
			}
		...
		setCellRenderer("logs", new CellRenderer({
				render: function(cell, value) { 
					var estado = editableGrid.getValueAt(cell.rowIndex,5);
					var carpeta = editableGrid.getValueAt(cell.rowIndex,7);
					if (estado.indexOf("Error")>0) {
						var rutaArchivon3 = "publicacion/Error/" + carpeta + "/" + value.toLowerCase();
						var rutaArchivon3 = btoa(encodeURIComponent(rutaArchivon3));
						cell.innerHTML = value ? "<a target='_blank' href='/pool-administracion-cargas/cargavistas/verarchivo/" + rutaArchivon3 + "' alt='datos.n3'><img border='0' alt='n3'  title=\"Ver Archivo de carga trazas\" src='" + image("file.png") + "'/></a>" : ""; 
					 } else if (estado.indexOf("Sin Procesar")>0) {		
						var rutaArchivon3 = "publicacion/NoProcesados/" + carpeta + "/" + value.toLowerCase();
						var rutaArchivon3 = btoa(encodeURIComponent(rutaArchivon3));
						cell.innerHTML = value ? "<a target='_blank' href='/pool-administracion-cargas/cargavistas/verarchivo/" + rutaArchivon3 + "' alt='datos.n3'><img border='0' alt='n3'  title=\"Ver Archivo de carga trazas\" src='" + image("file.png") + "'/></a>" : ""; 
					 } else {	
						var rutaArchivon3 = "publicacion/Procesados/" + carpeta + "/" + value.toLowerCase();
						var rutaArchivon3 = btoa(encodeURIComponent(rutaArchivon3));
						cell.innerHTML = value ? "<a target='_blank' href='/pool-administracion-cargas/cargavistas/verarchivo/" + rutaArchivon3 + "' alt='datos.n3'><img border='0' alt='n3'  title=\"Ver Archivo de carga trazas\" src='" + image("file.png") + "'/></a>" : ""; 		
					 }
					 cell.style.textAlign = "center"; 
		...
		setCellRenderer("valido", new CellRenderer({render: function(cell, value) {
			// this action will remove the row, so first find the ID of the row containing this cell 
			var estado = editableGrid.getValueAt(cell.rowIndex,5);

			if (estado.indexOf("Error")>0) {
				cell.innerHTML = "<img src=\"" + image("error.png") + "\" border=\"0\" alt=\"No valido\" title=\"No valido\"/>";
			} else {
			    cell.innerHTML= "<img src=\"" + image("valid.png") + "\" border=\"0\" alt=\"Valido\" title=\"Valido\"/>";
			}
		}}));
		...
		modelChanged = function(rowIndex, columnIndex, oldValue, newValue, row) { 
			...
			var ruta = "/pool-administracion-cargas/cargavistas/actualiza/" + valorid + "/" + nombreValor + "/" + valorValor;
			$.post(ruta)
			.done(function() {
		...
		rowRemoved = function(oldRowIndex, rowId) {
			var ruta = "/pool-administracion-cargas/cargavistas/borra/" + rowId;
			$.ajax(ruta, {
				data: { 
...
EditableGrid.prototype.onloadHTML = function(tableId) 
{
	...
	this.load({ metadata: [
	                       ...
	                       { name: "criterio", datatype: "string", editable: true },
...
EditableGrid.prototype.duplicate = function(rowIndex) 
{
	...
	var datos = btoa(encodeURIComponent(values['nombre']+ "|" + 
	                 ...
	                 values['logs']+ "|" +
					 values['archivos']));
	var ruta = "/pool-administracion-cargas/cargavistas/inserta/" + datos;
	$.post(ruta)
	.done(function() {
